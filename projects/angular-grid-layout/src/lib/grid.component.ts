...
import { KtdClientRect, getMutableClientRect } from './utils/client-rect';
import {
...
function getRowHeightInPixels(config: KtdGridCfg, height: number): number {
    ...
    return rowHeight === 'fit'
        ? ktdGetGridItemRowHeight(layout, height, gap)
        : rowHeight;
}
    ...
    implements OnChanges, AfterContentInit, AfterContentChecked, OnDestroy
{
    ...
    @ContentChildren(KtdGridItemComponent, { descendants: true })
    ...
    calculateRenderData(config = this.config) {
        const clientRect = (
            this.elementRef.nativeElement as HTMLElement
        ).getBoundingClientRect();
        // 劫持
        const box = this.filterFn(clientRect.width),
            ids = new Map(box.map((item, index) => [item.id, index]));
        // 劫持一般布局
        let copyConfig = JSON.parse(JSON.stringify(this.config));
        copyConfig.layout = copyConfig.layout.map((item) => {
            if (ids.has(item.id)) {
                return {
                    ...item,
                    ...box[ids.get(item.id) as number],
                };
            } else {
                return item;
            }
        ...
        this.gridCurrentHeight =
            ...
            (this.rowHeight === 'fit'
                ? clientRect.height
                : getGridHeight(copyConfig.layout, this.rowHeight, this.gap));
        ...
        this.setBackgroundCssVariables(
            getRowHeightInPixels(copyConfig, this.gridCurrentHeight)
        );
        // this.gridCurrentHeight =
        //     this.height ??
        //     (this.rowHeight === 'fit'
        //         ? clientRect.height
        //         : getGridHeight(this.layout, this.rowHeight, this.gap));
        // this._gridItemsRenderData = layoutToRenderItems(
        //     this.config,
        //     clientRect.width,
        //     this.gridCurrentHeight
    ...
    private setGridBackgroundVisible(visible: boolean) {
        const classList = (this.elementRef.nativeElement as HTMLDivElement)
            .classList;
        visible
            ? classList.add('ktd-grid-background-visible')
            : classList.remove('ktd-grid-background-visible');
    }
    ...
    private addGridItemAnimatingClass(
        ...
        return new Observable((observer) => {
            ...
            const handler = ((event: TransitionEvent) => {
                if (
                    !event ||
                    (event.target === gridItem.elementRef.nativeElement &&
    ...
    filterFn: (number) => Array<any> = () => [];
    // 注册布局过滤函数
    registerlayoutFilterFn(fn) {
        this.filterFn = typeof fn == 'function' ? fn : () => [];
    }
